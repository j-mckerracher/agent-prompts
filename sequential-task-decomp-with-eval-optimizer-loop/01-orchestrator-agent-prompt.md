...
story:
  ...
  acceptance_criteria_raw: |
    PASTE YOUR ACs HERE - one per line, any format
...
models:
  ...
  uow_decomposer: 'GPT-5.2-high-reasoning'
...
2. **Parse acceptance criteria** — Extract ACs from `acceptance_criteria_raw`, handling these formats:
   - `AC - <text>` or `AC: <text>`
   ...
   - Ignore blank lines
3. **Normalize to structured format** — Number as `AC1`, `AC2`, etc.
4. **Create intake artifacts** — Write `story.yaml`, `config.yaml`, `constraints.md`
5. **Confirm and proceed** — Show the user the normalized ACs and ask for confirmation before beginning TaskPlan stage
...
change_id: "4729040"
  ...
  model_assignments: {
    ...
    uow_plan_evaluator: "claude-haiku-4-5"
...
1. **State Machine Execution**: Execute the sequential workflow through all stages (Intake → TaskPlan → UoWPlan → Assignment → Execution → QA → UI QA)
...
- `Intake` → `TaskPlan` → `EvalTaskPlan` → `UoWPlan` → `EvalUoWPlan` → `Assign` → `EvalAssign` → `ExecuteUoWs` → `ImplementUoW` → `EvalImpl` → `WriteUnitTests` → `EvalUnitTests` → `WriteIntegrationTests` → `EvalIntegrationTests` → `NextUoW` → `QA` → `EvalQA` → `UIQA` (conditional) → `EvalUIQA` (conditional) → `PostQAReview` → `Remediation` → `Complete`
...
2. **If UI changes detected**:
   ...
   - Key mandate: Compare new/modified UI elements against existing baseline elements
...
| `uow_plan.yaml` | Compare UoW definitions, DoD items, dependencies |
...
{{artifact_root}}{CHANGE-ID}/
  ...
  logs/                         # Agent execution logs
    ...
    uow_decomposer/             # UoW Decomposer logs
