...
**⚠️ MANDATORY: ALL Cypress tests must pass. This includes BOTH component tests AND end-to-end tests.**
...
- **Location**: `{{e2e_tests_root}}/src/e2e`
- These are SEPARATE from component tests and must ALL pass

| Rating | Criteria                       |
| ------ | ------------------------------ |
| Pass   | **ALL** Cypress component tests pass **AND** **ALL** Cypress end-to-end tests pass |
| Fail   | One or more Cypress component tests fail **OR** one or more Cypress end-to-end tests fail |

**You MUST run both test suites and verify 100% pass rate before marking Cypress Gate as Pass.**
...
|------|---------------|----------------|
| Jest unit tests | `npm test` | Exit code 0, 100% pass rate |
| Cypress component tests | `npx cypress run --component` | Exit code 0, 100% pass rate |
| Cypress E2E tests | `npx cypress run` (in E2E app) | Exit code 0, 100% pass rate |
...
2. If Jest fails → **FAIL** immediately
3. If ANY Cypress test fails → **FAIL** immediately
...
**⚠️ CRITICAL REMINDER**: The Cypress Gate requires 100% pass rate for BOTH:
