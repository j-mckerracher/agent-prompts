...
3. Write Jest and Cypress tests
...
story:
  ...
  acceptance_criteria:             # Required: List your ACs
    - ""
    - ""
  examples:                        # Optional
    - description: ""
...
```
    ...
    ▼               ▼              ▼               ▼                       ▼
┌────────┐   ┌───────────┐   ┌──────────┐   ┌───────────┐           ┌──────────┐
│ Task   │   │   UoW     │   │ Software │   │   Test    │           │    QA    │
│Generat.│   │Decomposer │   │ Engineer │   │  Writers  │           │  Agent   │
└───┬────┘   └─────┬─────┘   └────┬─────┘   └─────┬─────┘           └────┬─────┘
    │              │              │               │                      │
    │              │              │         ┌─────┴─────┐                │
    │              │              │         │           │                │
...
└────────┘   │          │   │          │                   └──────────┘   └──────────┘
         ...
         ┌───────────────────────────────────────────────────────────────────────────────────┐
         │  Task Plan    │  UoW Plan   │  Assignment  │  Impl  │  Test  │  QA   │  UI QA    │
         │  Evaluator    │  Evaluator  │  Evaluator   │  Eval  │  Eval  │ Eval  │  Eval     │
         └───────────────────────────────────────────────────────────────────────────────────┘
                    ▲                          ▲                      ▲
...
```
┌─────────┐    ┌──────────┐    ┌──────────┐    ┌────────┐    ┌───────────┐    ┌────┐    ┌───────┐
│ Intake  │───▶│ TaskPlan │───▶│ UoW Plan │───▶│ Assign │───▶│ Execution │───▶│ QA │───▶│ UI QA │
└─────────┘    └──────────┘    └──────────┘    └────────┘    └───────────┘    └────┘    └───────┘
                    │               │              │              │              │           │
                    ▼               ▼              ▼              ▼              ▼           ▼
...
### Stage 3: UoW Decomposition
- **Agent**: UoW Decomposer
- **Evaluator**: UoW Plan Evaluator
- **Output**: `planning/uow_plan.yaml`
- **What happens**: Breaks tasks into atomic Units of Work with Definitions of Done

### Stage 4: Assignment
- **Agent**: Assignment Agent
- **Evaluator**: Assignment Evaluator
- **Output**: `planning/assignments.yaml`
- **What happens**: Schedules UoWs respecting dependencies, identifies parallelization opportunities
...
- **What happens**: Implements code changes, runs existing tests
...
- **What happens**: Validates all ACs with evidence, assesses regression risk, synthesizes knowledge
...
- **What happens**: Uses Playwright CLI to compare new/modified UI elements against existing baseline elements of the same type. Ensures visual and behavioral consistency across the application.
- **Key Mandate**: New UI elements must match the styling, behavior, and appearance of existing elements of the same type.
...
models:
  ...
  uow_decomposer: "claude-sonnet-4-5"
...
│   ├── uow_decomposer/     # UoW decomposition sessions
...
│   ├── uow_plan.yaml       # Units of Work with dependencies
...
| UoW Decomposer | `logs/uow_decomposer/` | `20260127_150000_session.yaml` |
...
| `03-uow-decomposer-agent-prompt.md` | UoW Decomposer | Decomposes into atomic units |
...
| `09-uow-plan-evaluator-prompt.md` | UoW Plan Evaluator | Evaluates UoW decomposition |
...
# 4. UoW Decomposer produces uow_plan.yaml (8 UoWs)
#    - UoW Plan Evaluator: REVISE (UOW-003 too large)
#    - UoW Decomposer revises → 10 UoWs
#    - UoW Plan Evaluator: PASS

# 5. Assignment produces assignments.yaml
#    - Assignment Evaluator: PASS

# 6. Execution loop (for each UoW):
