...
For greenfield projects with no established conventions, derive initial standards from PRD/plan docs and document them in the first scaffolding UoW.  
...
- **Never introduce new patterns, libraries, or conventions** when existing code already establishes how things should be done.  
  - For greenfield, define initial patterns only when explicitly called for in PRD/plan docs, then stick to them consistently.  
